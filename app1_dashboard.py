...
# 4. 하이브리드 AI 기획 빌더
# -----------------------------------------------------------------------------
def generate_marketing_plan(raw_context, user_keyword, platform_type="blog"):
    api_key, key_path, status_code = get_gemini_api_key_details()
    
    if status_code == "not_found":
        return {"error": f"API 키 파일이 존재하지 않습니다.\n확인할 경로: {key_path}"}
    elif status_code == "empty_file":
        return {"error": f"API 키 파일이 비어있습니다. 해당 텍스트 파일에 발급받으신 Gemini API 키를 넣어주세요.\n확인할 경로: {key_path}"}
    elif status_code.startswith("read_error"):
        return {"error": f"API 키 파일을 읽는 중 인코딩 에러가 발생했습니다.\n{status_code}"}
...
def run_application_layout(platform_name, platform_key, save_filename, dest_dir):
    ...
    if st.button("🚀 실시간 시장 분석 및 고효율 마케팅 기획 시작", key=f"action_{platform_key}"):
        ...
        with st.status("🔍 네이버/구글 실시간 동향 수집 및 기획안 생성 중...", expanded=True) as status:
            ...
            result = generate_marketing_plan(raw_context, user_keyword, platform_type=platform_key)
